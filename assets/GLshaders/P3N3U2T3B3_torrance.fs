...
// metallic
uniform int metallic_isImg;
uniform vec3 metallic_c;
uniform sampler2D metallic_i;
// roughness
uniform int roughness_isImg;
...
void main() {
    ...
    if (metallic_isImg != 0) metallic = texture(metallic_i, vs_out.uv).r;
    float roughness = roughness_c.r;
    if (roughness_isImg != 0) roughness = texture(roughness_i, vs_out.uv).r;
    float ao = 1.0f;
    if (ao_isImg != 0) ao = texture(ao_i, vs_out.uv).r;
